...
def create_department(session: Session):
    unique_des: bool = False
    while not unique_des:
        ...
        des_count: int = session.query(Department).filter(Department.description == description).count()
        unique_des = des_count == 0
        if not unique_des:
            print("We already have a student by that name.  Try again.")

    department = Department(abbreviation, chairName, building, office, description)
...
def select_department(session: Session):
    ...
    selection = input("Enter the abbreviation, chair name, building, office, or description of the department: ")
    department = session.query(Department).filter(
        (Department.abbreviation == selection) |
        (Department.chair_name == selection) |
        (Department.building == selection) |
        (Department.office == selection) |
        (Department.description == selection)
    ).first()
...
def select_department_abbreviation(sess: Session) -> Department:
    """
    Prompt the user for a specific student by the abbreviation.  Generally
    this is not a terribly useful approach, but I have it here for
    an example.
    :param sess:    The connection to the database.
    ...
    abbreviation: str = ''
    while not found:
        abbreviation = input("Department abbreviation to delete--> ")
        abb_count: int = sess.query(Department).filter(Department.abbreviation == abbreviation).count()
        found = abb_count == 1
        if not found:
            print("No department with that abbreviation.  Try again.")
    department: Department = sess.query(Department).filter(Department.abbreviation == abbreviation).first()
    return department


def select_department_chair(sess: Session) -> Department:
    """
    Select a department by the chair.
    :param sess:    The connection to the database.
    :return:        The selected department.
    """
    found: bool = False
    chairName: str = ''
    while not found:
        chairName = input("Department chair name to delete--> ")
        name_count: int = sess.query(Department).filter(Department.chairName == chairName).count()
        found = name_count == 1
        if not found:
            print("No department by that chair.  Try again.")
    department = sess.query(Student).filter(Department.chairName == chairName).first()
    return department


def select_department_location(sess: Session) -> Department:
    """
    Select a department by the building and office.
    :param sess:    The connection to the database.
    :return:        The selected Department.
    """
    found: bool = False
    ...
    while not found:
        ...
        name_count: int = sess.query(Department).filter(Department.building == building,
                                                        Department.office == office).count()
        found = name_count == 1
        if not found:
            print("No department by that location.  Try again.")
    department = sess.query(Department).filter(Department.building == building,
...
def select_department_description(sess: Session) -> Department:
    ...
    description: str = ''
    while not found:
        description = input("Department description to delete--> ")
        name_count: int = sess.query(Department).filter(Department.description == description).count()
        found = name_count == 1
        if not found:
            print("No department by that chair.  Try again.")
    department = sess.query(Student).filter(Department.description == description).first()
    return department
...
def find_department(sess: Session) -> Department:
    ...
    match find_department_command:
        case "abbreviation":
            department = select_department_abbreviation(sess)
        case "chair Name":
            department = select_department_chair(sess)
        case "building/office":
            department = select_department_location(sess)
        case "description":
            department = select_department_description(sess)
        case _:
            department = None
    return department
